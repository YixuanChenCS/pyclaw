...
website_files = [
    ...
    "pyclaw/website/share/index.md",
    "pyclaw/website/_includes/head_custom.html",
    "pyclaw/website/_includes/home.css",
    "pyclaw/website/docs/leaderboards/index.md",
]

exclude_files = [
    "pyclaw/website/install.ps1",
    "pyclaw/website/install.sh",
]
...
def blame(start_tag, end_tag=None):
    ...
    files = [
        ...
        or (f.startswith("pyclaw/resources/") and f.endswith(".yml"))
    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(pyclaw)" in author.lower():
                    pyclaw_total += count

    total_lines = sum(grand_total.values())
    pyclaw_percentage = (pyclaw_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, pyclaw_total, pyclaw_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if lower_subject.startswith("pyclaw:") or "co-authored-by: pyclaw" in lower_full:
            author += " (pyclaw)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, pyclaw_total, pyclaw_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "pyclaw_total": pyclaw_total,
                "pyclaw_percentage": round(pyclaw_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get pyclaw/non-pyclaw blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print pyclaw percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- Pyclaw wrote {round(pyclaw_percentage)}% of the code in this release.")
