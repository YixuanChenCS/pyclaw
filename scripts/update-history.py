...
def get_latest_version_from_history():
    ...
    match = re.search(r"### Pyclaw v(\d+\.\d+\.\d+)", history_content)
    if not match:
...
def main():
    ...
    version_header = f"### Pyclaw v{latest_ver}"
    ...
    next_version_idx = history_content.find("\n### Pyclaw v", version_idx + len(version_header))
    ...
    # Run blame to get pyclaw percentage
    blame_result = subprocess.run(["python3", "scripts/blame.py"], capture_output=True, text=True)
    pyclaw_line = blame_result.stdout.strip().split("\n")[-1]  # Get last line with percentage

    # Construct and run the pyclaw command
    message = history_prompt.format(pyclaw_line=pyclaw_line)
    ...
    # Run update-docs.sh after pyclaw
