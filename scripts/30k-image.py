...
# flake8: noqa: E501
"""
Generate a celebratory SVG image for Pyclaw reaching 30,000 GitHub stars.
...
def embed_font():
    ...
    font_path = (
        Path(__file__).parent.parent / "pyclaw" / "website" / "assets" / "Glass_TTY_VT220.ttf"
    )
...
def generate_confetti(count=150, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT):
    ...
    confetti = []
    colors = [PYCLAW_GREEN, PYCLAW_BLUE, GOLD_COLOR, "#e74c3c", "#9b59b6", "#3498db", "#2ecc71"]
...
<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <style>
    ...
    .pyclaw-logo {{ font-family: 'GlassTTYVT220', monospace; font-size: 120px; fill: {PYCLAW_GREEN}; text-anchor: middle; filter: url(#glow); }}
  ...
  <g clip-path="url(#rounded-rect)">
    ...
    <!-- Main content -->
    <text x="{width/2}" y="{height/2 - 100}" class="pyclaw-logo">pyclaw</text>
    ...
    <text x="{width/2}" y="{height - 50}" class="footer">github.com/Pyclaw-AI/pyclaw</text>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a celebration SVG for Pyclaw's 30K GitHub stars"
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="pyclaw-30k-stars.svg",
        help="Output file path (default: pyclaw-30k-stars.svg)",
    )
    parser.add_argument(
        "--width",
        "-w",
