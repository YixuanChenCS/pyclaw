...
# Pyclaw benchmark harness

Pyclaw uses benchmarks to quantitatively measure how well it works
with various LLMs.
...
This
benchmark evaluates how effectively pyclaw and LLMs can translate a
...
See [this writeup for a longer discussion about the benchmark](https://pyclaw.chat/2024/12/21/polyglot.html).
...
There are 3 main tasks involved in benchmarking pyclaw:
...
```
# Clone the pyclaw repo
git clone https://github.com/Pyclaw-AI/pyclaw.git

# Create the scratch dir to hold benchmarking results inside the main pyclaw dir:
cd pyclaw
mkdir tmp.benchmarks

# Clone the repo with the exercises
git clone https://github.com/Pyclaw-AI/polyglot-benchmark tmp.benchmarks/polyglot-benchmark
...
# Inside the container, install pyclaw as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `pyclaw`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `pyclaw`. When working with an experimental LLM, I recommend starting with `whole`
...
- `--read-model-settings=<filename.yml>` specify model settings, see here: https://pyclaw.chat/docs/config/adv-model-settings.html#model-settings
...
[pyclaw leaderboard data files](https://github.com/Pyclaw-AI/pyclaw/blob/main/pyclaw/website/_data/).
...
[pyclaw leaderboard data files](https://github.com/Pyclaw-AI/pyclaw/blob/main/pyclaw/website/_data/).
- These scripts are not intended for use by typical pyclaw end users.
