...
from pyclaw import models, sendchat
from pyclaw.coders import Coder, base_coder
from pyclaw.dump import dump  # noqa: F401
from pyclaw.io import InputOutput

BENCHMARK_DNAME = Path(os.environ.get("PYCLAW_BENCHMARK_DIR", "tmp.benchmarks"))
...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .pyclaw.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_pyclaw: bool = typer.Option(False, "--no-pyclaw", help="Do not run pyclaw"),
    ...
    read_model_settings: str = typer.Option(
        None, "--read-model-settings", help="Load pyclaw model settings from YAML file"
    ),
    ...
    if "PYCLAW_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.pyclaw.chat.history.md")
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.pyclaw.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.pyclaw.results.json"]
...
def summarize_results(dirname, stats_languages=None):
    ...
    if variants["model"]:
        ...
        command = f"pyclaw --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:pyclaw/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".pyclaw.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".pyclaw.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".pyclaw.chat.history.md"
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/pyclaw/benchmark/npm-test.sh"],
        ".cpp": ["/pyclaw/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
