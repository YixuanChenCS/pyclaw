...
from pyclaw import prompts, utils
...
class GitRepo:
    repo = None
    pyclaw_ignore_file = None
    pyclaw_ignore_spec = None
    pyclaw_ignore_ts = 0
    pyclaw_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if pyclaw_ignore_file:
            self.pyclaw_ignore_file = Path(pyclaw_ignore_file)

    def commit(self, fnames=None, context=None, message=None, pyclaw_edits=False, coder=None):
        """
        ...
        Args:
            ...
            pyclaw_edits (bool, optional): Whether the changes were made by Pyclaw. Defaults to False.
        ...
        Pyclaw generated the changes (`pyclaw_edits`).
        ...
        - pyclaw_edits=True: Changes were generated by Pyclaw (LLM).
        - pyclaw_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (pyclaw)".
        - --attribute-committer: Modify Committer name to "User Name (pyclaw)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: pyclaw (<model>) <pyclaw@pyclaw.chat>" trailer to commit message.

        Behavior Summary:

        1. When pyclaw_edits = True (AI Changes):
        ...
        2. When pyclaw_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as Pyclaw runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(pyclaw),
          Committer=You(pyclaw)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(pyclaw), Committer=You, Trailer added
        - User commit (defaults): pyclaw_edits=False -> Author=You, Committer=You(pyclaw)
        - User commit with explicit no-committer: pyclaw_edits=False,
        ...
        prefix_commit_message = pyclaw_edits and (
        ...
        if pyclaw_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: pyclaw ({model_name}) <pyclaw@pyclaw.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to pyclaw edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            pyclaw_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of pyclaw_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an pyclaw edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (pyclaw_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "pyclaw: " + commit_message
        ...
        committer_name = f"{original_user_name} (pyclaw)"
    ...
    def refresh_pyclaw_ignore(self):
        if not self.pyclaw_ignore_file:
            return

        current_time = time.time()
        if current_time - self.pyclaw_ignore_last_check < 1:
            return

        self.pyclaw_ignore_last_check = current_time

        if not self.pyclaw_ignore_file.is_file():
            return

        mtime = self.pyclaw_ignore_file.stat().st_mtime
        if mtime != self.pyclaw_ignore_ts:
            self.pyclaw_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.pyclaw_ignore_file.read_text().splitlines()
            self.pyclaw_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.pyclaw_ignore_file or not self.pyclaw_ignore_file.is_file():
            return False
        ...
        return self.pyclaw_ignore_spec.match_file(fname)
