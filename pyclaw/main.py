...
from pyclaw import __version__, models, urls, utils
from pyclaw.analytics import Analytics
from pyclaw.args import get_parser
from pyclaw.coders import Coder
from pyclaw.coders.base_coder import UnknownEditFormat
from pyclaw.commands import Commands, SwitchCoder
from pyclaw.copypaste import ClipboardWatcher
from pyclaw.deprecated import handle_deprecated_model_args
from pyclaw.format_settings import format_settings, scrub_sensitive_info
from pyclaw.history import ChatSummary
from pyclaw.io import InputOutput
from pyclaw.llm import litellm  # noqa: F401; properly init litellm on launch
from pyclaw.models import ModelSettings
from pyclaw.onboarding import offer_openrouter_oauth, select_default_model
from pyclaw.repo import ANY_GIT_ERROR, GitRepo
from pyclaw.report import report_uncaught_exceptions
from pyclaw.versioncheck import check_version, install_from_main_branch, install_upgrade
from pyclaw.watch import FileWatcher
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run pyclaw in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track pyclaw's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".pyclaw"):
            patterns_to_add.append(".pyclaw*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the pyclaw browser feature",
        ["pyclaw[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/Pyclaw-AI/pyclaw/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/Pyclaw-AI/pyclaw/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".pyclaw.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading pyclaw model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".pyclaw" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("pyclaw.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".pyclaw.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Pyclaw only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run pyclaw --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".pyclaw.conf.yml")
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "Pyclaw respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            io.tool_output(f"For more info: {urls.analytics}")
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve pyclaw?"
            )
    ...
    if args.pyclawignore:
        ignores.append(args.pyclawignore)
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".pyclaw" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install pyclaw properly?")
