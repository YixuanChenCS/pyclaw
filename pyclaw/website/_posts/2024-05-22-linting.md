...
excerpt: Pyclaw now lints code after every LLM edit and automatically fixes errors, using tree-sitter and AST-aware code context.
...
[![Linting code](/assets/linting.jpg)](https://pyclaw.chat/assets/linting.jpg)

# Linting code for LLMs with tree-sitter

Pyclaw now lints your code after every LLM edit, and offers to automatically fix
any linting errors.
You can also use pyclaw's lint-and-fix functionality on your source files any time
...
Pyclaw ships with basic linters built with tree-sitter that support
...
You can also configure pyclaw to use your preferred linters.
This allows pyclaw to check for a larger class of problems, keep the code style
aligned with the rest of your team, etc.

## Linting and fixing your code

Pyclaw now lints each source file after it applies the edits
suggested by an LLM.
If problems are found, pyclaw will ask if you'd like it to
attempt to fix the errors.
If so, pyclaw will send the LLM a report of the lint errors
and request changes to fix them. This process may iterate a few times
as the LLM works to fully resolve all the issues.

You can also lint and fix files any time, on demand from within the pyclaw chat or via the
command line:
...
- From the command line, you can run `pyclaw --lint` to lint and fix
...
linting errors.
LLM's don't have the luxury of using an IDE, so pyclaw sends
...
Pyclaw augments the raw linter by
displaying and
highlighting the lines that have errors within their
containing functions, methods, classes.
To do this, pyclaw uses tree-sitter to obtain the code's AST and analyzes it
...
Pyclaw comes batteries-included with built in linters for
[most popular programming languages](https://pyclaw.chat/docs/languages.html).
This provides wide support for linting without requiring
users to manually install a linter and configure it to work with pyclaw.

Pyclaw's built in language-agnostic linter uses tree-sitter to parse
...
Pyclaw simply uses these `ERROR` nodes to identify all the lines
