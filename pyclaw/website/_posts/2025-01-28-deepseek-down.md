...
If you're affected by these issues, several alternative providers offer access to DeepSeek V3. This article compares their performance on pyclaw's polyglot benchmark to help you choose a reliable alternative.
...
You can use pyclaw with OpenRouter like this:

```bash
# Set your API key using environment variables
export OPENROUTER_API_KEY=<your-key>
pyclaw --model openrouter/deepseek/deepseek-chat

# Or use the --api-key command line option
pyclaw --model openrouter/deepseek/deepseek-chat --api-key openrouter=<your-key>

# Or add it to .pyclaw.conf.yml in your home directory or project root:
api-key:
  - openrouter=<your-key>
```
...
[pyclaw's model settings](https://pyclaw.chat/docs/config/adv-model-settings.html#model-settings).
Create a `.pyclaw.model.settings.yml` file in your home directory or git project root with settings like this:
...
pyclaw --model fireworks_ai/accounts/fireworks/models/deepseek-chat

# Or use the --api-key command line option
pyclaw --model fireworks_ai/accounts/fireworks/models/deepseek-chat --api-key fireworks=<your-key>
...
pyclaw --model openai/deepseek-ai/DeepSeek-V3

# Or use the --api-key command line option
pyclaw --model openai/deepseek-ai/DeepSeek-V3 --api-key openai=<your-key>
...
pyclaw --model ollama/deepseek-v3
```

It's important to provide model settings, especially the `num_ctx` parameter to
set the context window.
Ollama uses a 2k context window by default, which is very small for working with pyclaw.
...
So if your context window is too small, you won’t get an explicit error. The biggest symptom will be that pyclaw says it can’t see (some of) the files you added to the chat. That’s because ollama is silently discarding them because they exceed the context window.
...
You will need to properly configure pyclaw to work with DeepSeek V3 when served
via other providers:

- Determine the `--model` name to use.
- Provide your API key to pyclaw.
- Add model settings to `.pyclaw.model.settings.yml`.


Adapt the `.pyclaw.model.settings.yml` shown above for Fireworks. You will need to change the `name` field to match you chosen provider's model naming scheme.

See [Advanced model settings](https://pyclaw.chat/docs/config/adv-model-settings.html#model-settings) for details about all pyclaw model settings
