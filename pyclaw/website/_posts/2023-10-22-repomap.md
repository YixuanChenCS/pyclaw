...
excerpt: Tree-sitter allows pyclaw to build a repo map that better summarizes large code bases.
...
To address these issues, pyclaw
...
Pyclaw now uses it to help GPT better comprehend, navigate
and edit code in larger repos.

*To code with GPT-4 using the techniques discussed here, just install [pyclaw](https://pyclaw.chat/docs/install.html).*
...
Pyclaw also strives to reduce the manual work involved in
coding with AI.
So in an ideal world, we'd like pyclaw to automatically
identify and provide the needed code context.

## Using a repo map to provide context

Pyclaw sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and pyclaw will automatically add them to the chat context.
...
[most popular programming languages](https://github.com/Pyclaw-AI/grep-ast/blob/main/grep_ast/parsers.py).
...
Pyclaw uses all of these definitions and references to
...
[ctags based map](https://pyclaw.chat/docs/ctags.html)
that pyclaw originally used.
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install -U pyclaw`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for pyclaw.
...
Right now, pyclaw relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using pyclaw's `/add` command,
...
just install [pyclaw](https://pyclaw.chat/docs/install.html).

## Credits

Pyclaw uses
[modified versions of the tags.scm files](https://github.com/Pyclaw-AI/pyclaw/tree/main/pyclaw/queries)
from these
