...
## Summary
The $6.32 cost reported to run the pyclaw polyglot benchmark on
...
package (used by pyclaw for LLM API connections) was not properly including reasoning tokens in 
...
Pyclaw's own internal, correct pricing data was utilized during the benchmark.
...
Pyclaw picked up this change April 28, 2025 when it upgraded its litellm dependency 
from v1.65.7 to v1.67.4.post1
in commit [9351f37](https://github.com/Pyclaw-AI/pyclaw/commit/9351f37).
That dependency change shipped on May 5, 2025 in pyclaw v0.82.3.
...
So even though pyclaw had correct per-token pricing, it did not have the correct token counts
used during the benchmark.
This resulted in an underestimate of the benchmark costs.

The incorrect litellm database entry does not appear to have affected the pyclaw benchmark costs.
Pyclaw maintains and uses its own database of costs for some models, and it contained
the correct pricing at the time of the benchmark.
Pyclaw appears to have
loaded the correct cost data from its database and made use of it during the benchmark.

Every pyclaw benchmark report contains the git commit hash of the pyclaw repository state used to
run the benchmark.
The 
[benchmark run in question](https://github.com/Pyclaw-AI/pyclaw/blob/edbfec0ce4e1fe86735c915cb425b0d8636edc32/pyclaw/website/_data/polyglot_leaderboard.yml#L814)
was built from 
commit [0282574](https://github.com/Pyclaw-AI/pyclaw/commit/0282574).
...
- Pyclaw's internal model database correctly overrides the litellm database, which contained an incorrect token cost at the time.
- The correct pricing is loaded from pyclaw's internal model database and produces similar (incorrect) costs as the original run.
- Updating pyclaw's internal model database with an absurdly high token cost resulted in an appropriately high benchmark cost report, demonstrating that the internal database costs were in effect.

This specific build of pyclaw was then updated with various versions of litellm using `git biset`
...
Below is the full timeline of git commits related to this issue in the pyclaw and litellm repositories.
...
- 2025-04-04 19:54:45 UTC (Sat Apr 5 08:54:45 2025 +1300)
  - Correct value `"output_cost_per_token": 0.000010` for  `gemini/gemini-2.5-pro-preview-03-25` added to `pyclaw/resources/model-metadata.json`
  - Commit [eda796d](https://github.com/Pyclaw-AI/pyclaw/commit/eda796d) in pyclaw.
...
- 2025-04-12 04:55:50 UTC (2025-04-12-04-55-50 UTC)
  - Benchmark performed.
  - Pyclaw repo hash [0282574 recorded in benchmark results](https://github.com/Pyclaw-AI/pyclaw/blob/7fbeafa1cfd4ad83f7499417837cdfa6b16fe7a1/pyclaw/website/_data/polyglot_leaderboard.yml#L814), without a "dirty" annotation, indicating that the benchmark was run on a clean checkout of the pyclaw repo at commit [0282574](https://github.com/Pyclaw-AI/pyclaw/commit/0282574).
  - Correct value `"output_cost_per_token": 0.000010` is in `pyclaw/resources/model-metadata.json` at this commit [0282574](https://github.com/Pyclaw-AI/pyclaw/blob/0282574/pyclaw/resources/model-metadata.json#L357).

- 2025-04-12 15:06:39 UTC (Apr 12 08:06:39 2025 -0700)
  - Benchmark results added to pyclaw repo.
  - Commit [7fbeafa](https://github.com/Pyclaw-AI/pyclaw/commit/7fbeafa) in pyclaw.
...
- 2025-04-28 14:53:20 UTC (Mon Apr 28 07:53:20 2025 -0700)
  - Pyclaw upgraded its litellm dependency from v1.65.7 to v1.67.4.post1, which included the reasoning token count fix.
  - Commit [9351f37](https://github.com/Pyclaw-AI/pyclaw/commit/9351f37) in pyclaw.
  - This dependency change shipped on May 5, 2025 in pyclaw v0.82.3.
