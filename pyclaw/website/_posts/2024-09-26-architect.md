...
Pyclaw now has experimental support for using two models to complete each coding task:
...
[pyclaw's code editing benchmark](/docs/benchmarks.html#the-benchmark).
...
This approach has recently become attractive for pyclaw due to 
...
incompatible with pyclaw's goal of providing an interactive,
pair programming AI coding experience.

## Code reasoning and code editing

Normally pyclaw asks the model to solve a coding problem in one prompt,
asking the LLM to explain the solution and return 
a well formatted series of file edits.
All of [pyclaw's editing formats](/docs/more/edit-formats.html)
require the LLM to return source code edits in a specific text
format, so that pyclaw can process the edits and apply them to the local source files.
...
The development version of pyclaw 
has built in defaults to support Architect/Editor coding with
o1-preview, o1-mini, GPT-4o and Claude 3.5 Sonnet.
Run pyclaw with `--architect` or get started quickly like this:

```
pip install -U pyclaw
...
pyclaw --sonnet --architect

# Work with OpenAI models, using gpt-4o as the Editor
export OPENAI_API_KEY=your-key-goes-here
pyclaw --4o --architect
pyclaw --o1-mini --architect
pyclaw --o1-preview --architect
```

## More info

Pyclaw has a number of "chat modes", and "architect" is available as a new chat mode.
The `--architect` switch is a shortcut for `--chat-mode architect`.
For more details, see documentation on 
[pyclaw's chat modes](/docs/usage/modes.html).
...
by itself in pyclaw's normal "code" editing mode
