...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var blameData = {
        labels: labels,
        datasets: [{
            label: 'Pyclaw\'s percent of new code by release',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.pyclaw_percentage }}, lines: {{ row.pyclaw_total }}, end_date: '{{ row.end_date }}' },{% endfor %}],
    ...
    var linesData = {
        ...
        datasets: [{
            label: 'Pyclaw',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.pyclaw_total }} },{% endfor %}],
            backgroundColor: 'rgba(54, 162, 235, 0.8)',
            borderColor: 'rgba(54, 162, 235, 1)',
            borderWidth: 1
        },
        {
            label: 'Human',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.total_lines | minus: row.pyclaw_total }} },{% endfor %}],
    ...
    var blameChart = new Chart(blameCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'Pyclaw\'s contribution';
                ...
                title: {
                    display: true,
                    text: 'Percent of new code written by pyclaw, by release',
                    font: {
                        size: 16
                    }
                }
