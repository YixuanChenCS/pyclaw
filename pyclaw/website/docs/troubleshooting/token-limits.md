...
Pyclaw will report an error **if a model responds** indicating that
...
For more info: https://pyclaw.chat/docs/token-limits.html
```

{: .note }
Pyclaw never *enforces* token limits, it only *reports* token limit errors
from the API provider.
The token counts that pyclaw reports are *estimates*.
...
It's best to only add the files that pyclaw will need to *edit*
...
as 4k tokens.
If you ask pyclaw to make a large change that affects a lot
...
Pyclaw has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://pyclaw.chat/docs/llms.html).
For serving local models, 
[Ollama](https://pyclaw.chat/docs/llms/ollama.html) is known to work well with pyclaw.

Try using pyclaw without an API proxy server
