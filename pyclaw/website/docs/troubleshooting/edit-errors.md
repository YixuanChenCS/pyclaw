...
In these cases, pyclaw might say something like "Failed to apply edit to *filename*"
or other error messages.

This usually happens because the LLM is disobeying the system prompts
and trying to make edits in a format that pyclaw doesn't expect.
Pyclaw makes every effort to get the LLM
...
Pyclaw also sends the LLM a [map of your entire git repo](https://pyclaw.chat/docs/repomap.html), so other relevant code will be included automatically.
...
Most local models are just barely capable of working with pyclaw,
so editing errors are probably unavoidable.

## Local models: context window and quantization

Be especially careful about the
[Ollama context window](https://pyclaw.chat/docs/llms/ollama.html#setting-the-context-window-size)
...
because they are not capable enough to follow pyclaw's system prompts.

## Try the whole edit format

Run pyclaw with `--edit-format whole` if were using a different edit format.
...
Run pyclaw with `--architect` or `/chat-mode architect` to enable [architect mode](../usage/modes.md#architect-mode-and-the-editor-model).
