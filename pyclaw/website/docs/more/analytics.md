...
Pyclaw can collect anonymous analytics to help
improve pyclaw's ability to work with LLMs, edit code and complete user requests.
...
Pyclaw collects information on:

- which LLMs are used and with how many tokens,
- which of pyclaw's edit formats are used,
...
This information helps improve pyclaw by identifying which models, edit formats,
...
If this is the first time you have enabled analytics, pyclaw
...
pyclaw will enable analytics for a random subset of users.
...
Pyclaw respects your privacy and never collects your code, prompts, chats, keys or any personal
info.
For more info: https://pyclaw.chat/docs/more/analytics.html
Allow collection of anonymous analytics to help improve pyclaw? (Y)es/(N)o [Yes]:
```
...
[sample analytics logs](https://github.com/pyclaw-ai/pyclaw/blob/main/pyclaw/website/assets/sample-analytics.jsonl).
These are the last 1,000 analytics events from the author's
personal use of pyclaw, updated regularly.


### Analytics code

Since pyclaw is open source, all the places where pyclaw collects analytics
are visible in the source code.
They can be viewed using 
[GitHub search](https://github.com/search?q=repo%3Apyclaw-ai%2Fpyclaw+%22.event%28%22&type=code).


### Logging and inspecting analytics

You can get a full log of the analytics that pyclaw is collecting,
in case you would like to audit or inspect this data.

```
pyclaw --analytics-log filename.jsonl
```

If you want to just log analytics without reporting them, you can do:

```
pyclaw --analytics-log filename.jsonl --no-analytics
```

### Sending analytics to custom PostHog project or installation

Pyclaw uses PostHog for analytics collection. You can configure pyclaw to send analytics to your own PostHog project or a custom PostHog installation using these parameters:
...
If you have concerns about any of the analytics that pyclaw is collecting
