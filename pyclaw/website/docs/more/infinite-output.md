...
parent: More info
nav_order: 480
description: Pyclaw can handle "infinite output" from models that support prefill.
---
...
Pyclaw is able to work around this limit with models that support
"prefilling" the assistant response.
When you use pyclaw with a model that supports prefill, you will see
...
When pyclaw is collecting code edits from a model and
it hits the output token limit,
pyclaw simply initiates another LLM request with the partial
response prefilled.
...
Pyclaw supports "infinite output" for models that support "prefill",
such as:
