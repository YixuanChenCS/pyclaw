...
In most cases, you can safely ignore pyclaw's warning about unknown context
window size and model costs.

{: .note }
Pyclaw never *enforces* token limits, it only *reports* token limit errors
from the API provider.
You probably don't need to
configure pyclaw with the proper token limits
for unusual models.

But, you can register context window limits and costs for models that aren't known
to pyclaw. Create a `.pyclaw.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch pyclaw.
...
in the `.pyclaw.model.metadata.json` file.
...
Pyclaw has a number of settings that control how it works with
different models.
...
a model that pyclaw doesn't know about.


### Configuration file locations

You can override or add settings for any model by creating a `.pyclaw.model.settings.yml` file in one of these locations:
...
You can use the special model name `pyclaw/extra_params` to define 
...
`pyclaw/extra_params` settings taking precedence for any direct conflicts.
...
[models.py](https://github.com/Pyclaw-AI/pyclaw/blob/main/pyclaw/models.py)
file for more details about all of the model setting that pyclaw supports.
...
<!--[[[cog
from pyclaw.models import get_model_settings_as_yaml
cog.out("```yaml\n")
