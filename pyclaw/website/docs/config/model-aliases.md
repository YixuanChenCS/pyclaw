...
You can define aliases when launching pyclaw using the `--alias` option:

```bash
pyclaw --alias "fast:gpt-4o-mini" --alias "smart:o3-mini"
```
...
Of course,
you can also define aliases in your [`.pyclaw.conf.yml` file](https://pyclaw.chat/docs/config/pyclaw_conf.html):
...
```bash
pyclaw --model fast  # Uses gpt-4o-mini
pyclaw --model smart  # Uses o3-mini
```
...
Pyclaw includes some built-in aliases for convenience:

<!--[[[cog
import cog
from pyclaw.models import MODEL_ALIASES
