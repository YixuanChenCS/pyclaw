...
parent: More info
nav_order: 400
description: You can script pyclaw via the command line or python.
---

# Scripting pyclaw

You can script pyclaw via the command line or python.

## Command line

Pyclaw takes a `--message` argument, where you can give it a natural language instruction.
It will do that one thing, apply the edits to the files and then exit.
So you could do:

```bash
pyclaw --message "make a script that prints hello" hello.js
```
...
```bash
for FILE in *.py ; do
    pyclaw --message "add descriptive docstrings to all the functions" $FILE
done
```

Use `pyclaw --help` to see all the 
...
--message COMMAND, --msg COMMAND, -m COMMAND
                      ...
                      (disables chat mode) [env var: PYCLAW_MESSAGE]
--message-file MESSAGE_FILE, -f MESSAGE_FILE
                      Specify a file containing the message to send GPT, process reply,
                      then exit (disables chat mode) [env var: PYCLAW_MESSAGE_FILE]
--yes                 Always say yes to every confirmation [env var: PYCLAW_YES]
...
--dirty-commits, --no-dirty-commits
                      ...
                      var: PYCLAW_DIRTY_COMMITS]
--dry-run, --no-dry-run
                      Perform a dry run without modifying files (default: False) [env var:
                      PYCLAW_DRY_RUN]
--commit              Commit all pending changes with a suitable commit message, then exit
                      [env var: PYCLAW_COMMIT]
```


## Python

You can also script pyclaw from python:
...
See the
[Coder.create() and Coder.__init__() methods](https://github.com/Pyclaw-AI/pyclaw/blob/main/pyclaw/coders/base_coder.py)
