...
description: Watch how pyclaw adds support for tons of new programming languages by integrating with tree-sitter-language-pack. Demonstrates using pyclaw to script downloading a collection of files, and using ad-hoc bash scripts to have pyclaw modify a collection of files.
---
...
- 0:01 We're going to add a ton of new languages to pyclaw via tree-sitter-language-pack.
...
- 1:45 We need to get all the tags files from each repository for pyclaw's repo-map. Let's have pyclaw write a script to fetch them all.
- 2:05 We'll show pyclaw the language definitions json file.
...
- 9:45 Let's have pyclaw add them all, and register each using their commonly used file extensions.
...
- 13:00 Ok, let's download all the tags files into the right spot in the pyclaw repo.
- 14:00 This will take a minute...
- 16:07 Delete some no-op or empty tags files.
- 16:16 Let's commit all the unmodified tags files.
- 16:33 We need to update each tag file, so that pyclaw can identify names of functions, classes, etc in all these languages.
- 17:01 Let's use a bash loop to script pyclaw to modify each tags file.
- 17:12 I'm giving pyclaw a read-only example of an already modified tags file, as an example to follow.
- 19:04 Looks like it correctly updated the first couple of tags files.
- 19:28 Let's grep to watch pyclaw's progress working through the list of files.
...
- 27:19 Each language needs a "fixture" with some sample code to parse during the test. Let's show pyclaw the layout of the fixtures directory.
- 27:50 We can use a bash loop to ask pyclaw to add test coverage for each new tags file.
- 28:12 We'll pass the fixtures directory listing to pyclaw.
- 28:52 Just need to fix the bash to correctly iterate through the list of tags files.
- 29:27 I forgot to ask pyclaw to actually generate a sample code fixture for each language.
...
- 31:52 Can pyclaw figure out what's wrong?
- 32:27 Well, pyclaw made the test pass by basically skipping Arduino.
...
- 32:54 Ok, now we're parsing Arduino code properly. Undo pyclaw's bogus test fix.
- 33:05 Ok, arduino passes now but there seems to be a regression with tsx?
- 33:20 Can pyclaw figure out why?
- 34:10 Let's check the parsers map.
- 35:00 Well, that's all for this recording. The tsx problem was due to a bad mapping from ".tsx" to "typescript" in the map that pyclaw generated earlier.
