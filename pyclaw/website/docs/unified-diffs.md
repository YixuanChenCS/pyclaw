...
Pyclaw now asks GPT-4 Turbo to use
...
Pyclaw's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using pyclaw's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **Pyclaw's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using pyclaw's existing "SEARCH/REPLACE block" edit format.
- **Pyclaw's new unified diff edit format raised June GPT-4's score to 59%**. 
...
Pyclaw's new unified diff editing format
...
numerous variations on pyclaw's existing editing formats,
...
pyclaw's new editing format and refactoring benchmark.
...
The design and implementation of pyclaw's new unified diff editing format
...
Pyclaw's [previous benchmark results](https://pyclaw.chat/docs/benchmarks.html) made
...
Pyclaw tells GPT not to include line numbers,
...
Pyclaw's system prompt encourages
...
When a patch fails, pyclaw needs to ask GPT for a corrected version of the diff.
...
Pyclaw tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, pyclaw uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on pyclaw's original Exercism benchmark.

## Refactoring benchmark

Pyclaw has long used a
[benchmark suite based on 133 Exercism python exercises](https://pyclaw.chat/2023/07/02/benchmarks.html).
...
[9 popular open source python repositories](https://github.com/Pyclaw-AI/refactor-benchmark)
...
A [simple python AST scanning script](https://github.com/Pyclaw-AI/pyclaw/blob/main/benchmark/refactor_tools.py)
...
[benchmark suite that provokes, detects and quantifies GPT coding laziness](https://github.com/Pyclaw-AI/refactor-benchmark).
...
pyclaw's new unified diff format seems
...
when originally building pyclaw.
...
incorporated into pyclaw provide
...
pyclaw's simple, high level style of unified diffs.
