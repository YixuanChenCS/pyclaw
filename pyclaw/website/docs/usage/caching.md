...
parent: Usage
nav_order: 750
description: Pyclaw supports prompt caching for cost savings and faster coding.
---

# Prompt caching

Pyclaw supports prompt caching for cost savings and faster coding.
Currently Anthropic provides caching for Sonnet and Haiku,
and DeepSeek provides caching for Chat.

Pyclaw organizes the chat history to try and cache:
...
Run pyclaw with `--cache-prompts` or add that setting to your 
...
When caching is enabled, it will be noted for the main model when pyclaw launches:
...
Pyclaw can ping the provider to keep your prompt cache warm and prevent
it from expiring.
By default, Anthropic keeps your cache for 5 minutes.
Use `--cache-keepalive-pings N` to tell pyclaw to ping
every 5 minutes to keep the cache warm.
Pyclaw will ping up to `N` times over a period of `N*5` minutes
