...
Pyclaw has a few different chat modes:

- `code` - Pyclaw will make changes to your code to satisfy your requests.
- `ask` - Pyclaw will discuss your code and answer questions about it, but never make changes.
- `architect` - Like code mode, pyclaw will change your files. An architect model will propose changes and an editor model will translate that proposal into specific file edits.
- `help` - Pyclaw will answer questions about pyclaw: usage, configuration, troubleshooting, etc.

By default, pyclaw starts in "code" mode. As you are talking, you can
...
Or you can launch pyclaw in one of the modes with the `--chat-mode <mode>` switch.
There is also a special shortcut `--architect` to launch in `--chat-mode architect`.

The pyclaw prompt will indicate the active mode:
...
Use ask mode to discuss what you want to do, get suggestions or options from pyclaw
and provide feedback on the approach.
Once pyclaw understands the mission, switch to code mode to have it start editing
your files.
...
have pyclaw execute on the plan you've been discussing.
...
When you are in architect mode, pyclaw sends your requests to two models:
...
2. Pyclaw then sends another request to an "editor model",
asking it to turn the architect's proposal into specific file editing instructions.
Pyclaw has built in defaults to select an editor model based on your main model.
...
The editor model uses one of pyclaw's edit formats to let the LLM
edit source files.
Pyclaw will pick a suitable edit format by default,
...
See this article on 
[pyclaw's architect/editor mode](/2024/09/26/architect.html)
for more details.
...
#### /help How do I use pyclaw with ollama?

Run `pyclaw --model ollama/<ollama-model>`.
See these docs for more info: https://pyclaw.chat/docs/llms/ollama.html
