...
Pyclaw can automatically lint and test your code
...
Pyclaw comes with built in linters for 
...
If there are linting errors, pyclaw expects the
command to print them on stdout/stderr
and return a non-zero exit code.
This is how most linters normally operate.

By default, pyclaw will lint any files which it edits.
...
confuse pyclaw into thinking there's an actual lint error that needs to be fixed.
...
Pyclaw will run the test command without any arguments.
If there are test errors, pyclaw expects the
command to print them on stdout/stderr
and return a non-zero exit code.

Pyclaw will try and fix any errors
if the command returns a non-zero exit code.

You can configure pyclaw to run your test suite
...
If you want to have pyclaw compile code after each edit, you
...
and optionally share the output with pyclaw.
This can be useful to share error messages or to show pyclaw
