...
parent: Usage
nav_order: 800
description: Tell pyclaw to follow your coding conventions when it works on your code.
---
...
The easiest way to do that with pyclaw is to simply create
...
and then we can add it to the pyclaw chat, along with the file(s)
that we want to edit.

It's best to load the conventions file with `/read CONVENTIONS.md` 
or `pyclaw --read CONVENTIONS.md`. 
This way it is marked as read-only, and cached if prompt caching
is enabled.

## Community contributed conventions

You can check the [pyclaw conventions repository](https://github.com/Pyclaw-AI/conventions)
to find or contribute conventions files.

## Always load conventions

You can also configure pyclaw to always load your conventions file
in the [`.pyclaw.conf.yml` config file](https://pyclaw.chat/docs/config/pyclaw_conf.html):
...
> $ pyclaw --read CONVENTIONS.md useragent.py  
>  
> Pyclaw v0.24.2-dev  
