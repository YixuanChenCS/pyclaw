...
parent: Usage
nav_order: 50
description: Control pyclaw with in-chat commands like /add, /model, etc.
---
...
Pyclaw supports commands from within the chat, which all start with `/`.

<!--[[[cog
from pyclaw.commands import get_help_md
cog.out(get_help_md())
]]]-->

|Command|Description|
|:------|:----------|
| **/add** | Add files to the chat so pyclaw can edit them or review them in detail |
...
| **/help** | Ask questions about pyclaw |
...
| **/undo** | Undo the last git commit if it was done by pyclaw |
...
It's always safe to use Control-C to interrupt pyclaw if it isn't providing a useful response. The partial response remains in the conversation, so you can refer to it when you reply to the LLM with more information or direction.
...
To use vi/vim keybindings, run pyclaw with the `--vim` switch.
