...
Pyclaw can connect to models provided by Amazon Bedrock.
To configure Pyclaw to use the Amazon Bedrock API, you need to set up your AWS credentials.
...
Once your AWS credentials are set up, you can run Pyclaw with the `--model` command line switch, specifying the Bedrock model you want to use:

```bash
# Change directory into your codebase
cd /to/your/project

pyclaw --model bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0
```

Sometimes it seems to help if you prefix the model name with "us.":

```bash
pyclaw --model bedrock/us.anthropic.claude-3-5-sonnet-20240620-v1:0
```
...
```bash
pyclaw --list-models bedrock/
```

Make sure you have access to these models in your AWS account before attempting to use them with Pyclaw.

## Install boto3
You may need to install the `boto3` package.

```bash
# If you installed with pyclaw-install or `uv tool`
uv tool run --from pyclaw pip install boto3

# Or with pipx...
pipx inject pyclaw boto3
