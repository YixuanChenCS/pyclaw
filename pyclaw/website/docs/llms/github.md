...
Pyclaw can connect to GitHub Copilot’s LLMs because Copilot exposes a standard **OpenAI-style**
endpoint at:
...
Each returned ID can be used with pyclaw by **prefixing it with `openai/`**:

```bash
pyclaw --model openai/gpt-4o
# or
pyclaw --model openai/claude-3.7-sonnet-thought
```
...
## Optional config file (`~/.pyclaw.conf.yml`)
...
* Calls made through pyclaw are billed through your Copilot subscription  
  (pyclaw will still print *estimated* costs).
* The Copilot docs explicitly allow third-party “agents” that hit this API – pyclaw is playing by
  the rules.
* Pyclaw talks directly to the REST endpoint—no web-UI scraping or browser automation.
