...
Pyclaw can connect to the OpenAI models on Azure.
...
Start working with pyclaw and Azure on your codebase:

```bash
# Change directory into your codebase
cd /to/your/project

pyclaw --model azure/<your_model_deployment_name>

# List models available from Azure
pyclaw --list-models azure/
```

Note that pyclaw will also use environment variables
like `AZURE_OPENAI_API_xxx`.

The `pyclaw --list-models azure/` command will list all models that pyclaw supports through Azure, not the models that are available for the provided endpoint.
