...
Start working with pyclaw and Anthropic on your codebase:

```bash
# Change directory into your codebase
cd /to/your/project

# Pyclaw uses Claude 3.7 Sonnet by default
pyclaw

# List models available from Anthropic
pyclaw --list-models anthropic/
```
...
You can use `pyclaw --model <model-name>` to use any other Anthropic model.
For example, if you want to use a specific version of Opus
you could do `pyclaw --model claude-3-opus-20240229`.

## Thinking tokens

Pyclaw can work with Sonnet 3.7's new thinking tokens, but does not ask Sonnet to use
thinking tokens by default.

Enabling thinking currently requires manual configuration.
You need to add the following to your `.pyclaw.model.settings.yml` 
