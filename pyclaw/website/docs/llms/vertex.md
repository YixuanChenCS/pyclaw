...
Pyclaw can connect to models provided by Google Vertex AI.
...
To configure Pyclaw to use the Vertex AI API, you need to set `VERTEXAI_PROJECT` (the GCP project ID)
and `VERTEXAI_LOCATION` (the GCP region) [environment variables for Pyclaw](/docs/config/dotenv.html).
...
Start working with pyclaw and Vertex AI on your codebase:

```
# Change directory into your codebase
cd /to/your/project

pyclaw --model vertex_ai/claude-3-5-sonnet@20240620
```

Or you can use the [YAML config](/docs/config/pyclaw_conf.html) to set the model to any of the 
models supported by Vertex AI.

Example `.pyclaw.conf.yml` file:
