...
from pyclaw import __version__, models, prompts, urls, utils
from pyclaw.analytics import Analytics
from pyclaw.commands import Commands
from pyclaw.exceptions import LiteLLMExceptions
from pyclaw.history import ChatSummary
from pyclaw.io import ConfirmGroup, InputOutput
from pyclaw.linter import Linter
from pyclaw.llm import litellm
from pyclaw.models import RETRY_TIMEOUT
from pyclaw.reasoning_tags import (
    REASONING_TAG,
...
from pyclaw.run_cmd import run_cmd
from pyclaw.utils import format_content, format_messages, format_tokens, is_image_file
from pyclaw.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_pyclaw_commit_hash = None
    pyclaw_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import pyclaw.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                pyclaw_commit_hashes=from_coder.pyclaw_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"Pyclaw v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .pyclawignore"
                )
    ...
    def __init__(
        ...
        if pyclaw_commit_hashes:
            self.pyclaw_commit_hashes = pyclaw_commit_hashes
        else:
            self.pyclaw_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches pyclawignore spec.")
                continue
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("PYCLAW_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.pyclaw_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, pyclaw_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_pyclaw_commit_hash = commit_hash
        self.pyclaw_commit_hashes.add(commit_hash)
        self.last_pyclaw_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each pyclaw commit.")
