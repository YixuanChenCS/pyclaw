...
from pyclaw.coders.udiff_coder import find_diffs
...
1. Add the `--check-update` argument to the argument parser in `pyclaw/main.py`.
2. Modify the `check_version` function in `pyclaw/versioncheck.py` to return a boolean indicating whether an update is available.
3. Use the returned value from `check_version` in `pyclaw/main.py` to set the exit status code when `--check-update` is used.

Here are the diffs for those changes:

```diff
--- pyclaw/versioncheck.py
+++ pyclaw/versioncheck.py
@@ ... @@
